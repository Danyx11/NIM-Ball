...
`frame.webp` itself is generated, not hand-painted as one flat image: `scripts/bake_arena.py` composites it from separate source layers in `design/arena/` (ice, wood ring + "liant" shadow tail, goal-post bars, scoreboard plaque) using the exact alignment math from `design-lab/main.js` (`alignPoutresToPhysics`, `alignBorduresButs`, `alignScoreboardWood`, `drawFieldLines`), just evaluated once against the fixed `FX0..FY1` physics rect instead of a live viewport rect. Re-run it (`python3 scripts/bake_arena.py`) after touching any of those source layers; it needs Pillow + numpy (`pip install pillow numpy`, not an npm dependency).
