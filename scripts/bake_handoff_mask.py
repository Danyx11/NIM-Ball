...
Only a light blend toward white (WHITE_BLEND) — the natural ice
scratches/cracks stay visible on purpose (per feedback: an earlier, much
stronger blend + blur pass read as a flat white rectangle instead of ice).
...
Usage: python3 scripts/bake_handoff_mask.py
"""
...
BLUR_RADIUS = 0         # kept at 0 so the ice's own cracks/scratches stay crisp
WHITE_BLEND = 0.28      # 0 = untouched crop, 1 = flat white — light lift only


def main():
    ice = Image.open(ICE_SRC).convert("RGB")
    crop = ice.crop((NOTCH_X0, FY0, NOTCH_X1, FY1))
    base = crop.filter(ImageFilter.GaussianBlur(BLUR_RADIUS)) if BLUR_RADIUS else crop
    white = Image.new("RGB", base.size, (255, 255, 255))
    baked = Image.blend(base, white, WHITE_BLEND)
